...
<head>
    ...
    <title>Laporan Kehadiran Pustakawan Struktural Bulan {{ $namaBulan }} {{ $tahun }}</title>
    <style>
        h2 {
            text-align: center;
            font-size: 16px;
...
<body>
    ...
    <table class="rekap">
        ...
        <tbody>
            ...
            @endphp
            @foreach ($pustakawan as $index => $item)
                @php
                ...
                <tr>
                    ...
                    <td>{{ number_format($totalBarokahJabatan ?? 0, 0, ',', '.') }}</td>
                    <td>{{ number_format($TMT2 ?? 0, 0, ',', '.') }}</td>
                    <td>{{ number_format($totalKehadiran ?? 0, 0, ',', '.') }}</td>
                    <td>{{ number_format($item->t_tunkel_id ?? 0, 0, ',', '.') }}</td>
                    <td>{{ number_format($item->t_anak_id ?? 0, 0, ',', '.') }}</td>
                    <td>{{ number_format($rankDosen ?? 0, 0, ',', '.') }}</td>
                    <td>{{ number_format($kehormatan ?? 0, 0, ',', '.') }}</td>

                    <td>{{ number_format($jumlahKeseluruhan ?? 0, 0, ',', '.') }}</td>
                    ...
                    <td class="keterangan">{{ $persentase }}%</td>
                </tr>
            @endforeach
            ...
            <tr style="background:#eef1f7; font-weight:bold;">
                ...
                <td>{{ number_format($barokahJabatanTotal ?? 0, 0, ',', '.') }}</td>
                <td>{{ number_format($totalPengabdian ?? 0, 0, ',', '.') }}</td>
                <td>{{ number_format($jumlahKehadiran ?? 0, 0, ',', '.') }}</td>
                <td>{{ number_format($pustakawan->sum('t_tunkel_id'), 0, ',', '.') }}</td>
                <td>{{ number_format($pustakawan->sum('t_anak_id'), 0, ',', '.') }}</td>
                <td>{{ number_format($totalRankDosen ?? 0, 0, ',', '.') }}</td>
                <td>{{ number_format($totalKehormatan ?? 0, 0, ',', '.') }}</td>

                <td>{{ number_format($totalJumlahSemua ?? 0, 0, ',', '.') }}</td>
                <td style="border:1px solid #000; text-align: center;" colspan="6">TOTAL
                    {{ number_format($totalJumlahSemua ?? 0, 0, ',', '.') }}</td>
            </tr>
        </tbody>
    </table>
    ...
    <div class="page-break">
        <h2>REKAPITULASI DAFTAR HADIR UMANA' <br> PERPUSTAKAAN IBRAHIMY <br> TAHUN {{ $tahun }}</h2>
    </div>
        ...
        <tbody class="rekap">
            @foreach ($hadirPagi as $item)
                ...
                <tr>
                    ...
                    <td class="pustakawan">{{ $item->nama_pustakawan }}</td>
                    @foreach ($dates as $tgl)
                        @php
    ...
    <br>
    {!! renderLiburTable($liburs, $bulan, $tahun, 'PAGI') !!}
    ...
    <br>
    {!! renderLiburTable($liburs, $bulan, $tahun, 'SIANG') !!}
    ...
    {!! renderLiburTable($liburs, $bulan, $tahun, 'MALAM') !!}
