...
<div class="modal fade" id="kt_modal_new_target{{ $item->id }}" tabindex="-1" aria-hidden="true">
    ...
    <div class="modal-dialog modal-dialog-centered mw-800px">
        ...
        <div class="pt-0 modal-body scroll-y px-15 px-lg-15 pb-15">
            <!--begin:Form-->
            <form class="form" method="POST" enctype="multipart/form-data" action="{{ route('struktural-izin.store') }}">
                @csrf
                <!--begin::Heading-->
                ...
                <div class="mb-6">
                    ...
                    <select class="form-select" name="pustakawan_id" data-control="select2" data-hide-search="true" data-placeholder="Pilih Pustakawan" required>
                        <option value="" disabled selected>Pilih Pustakawan</option>
                         @foreach ($izinPustakawan as $item)
                            <option value="{{ $item->id }}">{{ $item->pustakawan->nama_pustakawan }}</option>
                        @endforeach
                    </select>
                </div>
                <!--end::Input group-->
